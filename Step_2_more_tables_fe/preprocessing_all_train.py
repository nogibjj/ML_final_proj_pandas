...
train_static = pl.concat(
    [
        pl.read_csv(pathway + "csv_files/train/train_static_0_0.csv").pipe(
            set_table_dtypes
        ),
        pl.read_csv(pathway + "csv_files/train/train_static_0_1.csv").pipe(
            set_table_dtypes
        ),
    ],
    how="vertical_relaxed",
)
train_static_cb = pl.read_csv(pathway + "csv_files/train/train_static_cb_0.csv").pipe(
    set_table_dtypes
)
train_person_1 = pl.read_csv(pathway + "csv_files/train/train_person_1.csv").pipe(
    set_table_dtypes
)
train_credit_bureau_b_2 = pl.read_csv(
    pathway + "csv_files/train/train_credit_bureau_b_2.csv"
).pipe(set_table_dtypes)

# Additional depth=1 files
train_other_1 = pl.read_csv(pathway + "csv_files/train/train_other_1.csv").pipe(
    set_table_dtypes
)
train_credit_bureau_b_1 = pl.read_csv(
    pathway + "csv_files/train/train_credit_bureau_b_1.csv"
).pipe(set_table_dtypes)
train_deposit_1 = pl.read_csv(pathway + "csv_files/train/train_deposit_1.csv").pipe(
    set_table_dtypes
)
# test_debitcard_1 = pl.read_csv(pathway + "csv_files/test/test_debitcard_1.csv").pipe(set_table_dtypes)

# casting date columns to date
...
join_data1 = (
    train_basetable.join(train_static, how="left", on="case_id")
...
drop_cols = missing_values(join_data1.to_pandas(), threshold=0.95)
...
# 2nd Set of Tables
# Additional training data, depth = 1
train_applprev_1 = pl.concat(
    [
        pl.read_csv(pathway + "csv_files/train/train_applprev_1_0.csv").pipe(
            set_table_dtypes
        ),
        pl.read_csv(pathway + "csv_files/train/train_applprev_1_1.csv").pipe(
            set_table_dtypes
        ),
    ],
    how="vertical_relaxed",
...
train_tax_registry_a_1 = pl.read_csv(
    pathway + "csv_files/train/train_tax_registry_a_1.csv"
).pipe(set_table_dtypes)
train_tax_registry_b_1 = pl.read_csv(
    pathway + "csv_files/train/train_tax_registry_b_1.csv"
).pipe(set_table_dtypes)
train_tax_registry_c_1 = pl.read_csv(
    pathway + "csv_files/train/train_tax_registry_c_1.csv"
).pipe(set_table_dtypes)

train_credit_bureau_a_1 = pl.concat(
    [
        pl.read_csv(pathway + "csv_files/train/train_credit_bureau_a_1_0.csv").pipe(
            set_table_dtypes
        ),
        pl.read_csv(pathway + "csv_files/train/train_credit_bureau_a_1_1.csv").pipe(
            set_table_dtypes
        ),
        pl.read_csv(pathway + "csv_files/train/train_credit_bureau_a_1_2.csv").pipe(
            set_table_dtypes
        ),
        pl.read_csv(pathway + "csv_files/train/train_credit_bureau_a_1_3.csv").pipe(
            set_table_dtypes
        ),
    ],
    how="vertical_relaxed",
...
train_applprev_1_feats_2 = (
    train_applprev_1.select(
        [
            "case_id",
            "num_group1",
            "credtype_587L",
...
train_tax_registry_a_1_feats = train_tax_registry_a_1.group_by("case_id").agg(
    pl.col("amount_4527230A").sum().alias("amount_4527230A_sum")
)

train_tax_registry_b_1_feats = train_tax_registry_b_1.group_by("case_id").agg(
    pl.col("amount_4917619A").sum().alias("amount_4917619A_sum")
)

train_tax_registry_c_1_feats = train_tax_registry_c_1.group_by("case_id").agg(
...
train_tax_registry_c_1_feats = train_tax_registry_c_1_feats.with_columns(
    pl.col("case_id").cast(pl.Int64)
)

join_data2 = (
    train_basetable.join(train_applprev_1_feats_1, how="left", on="case_id")
    .join(train_applprev_1_feats_2, how="left", on="case_id")
    .join(train_tax_registry_a_1_feats, how="left", on="case_id")
    .join(train_tax_registry_b_1_feats, how="left", on="case_id")
    .join(train_tax_registry_c_1_feats, how="left", on="case_id")
    .join(train_credit_bureau_a_1_feats, how="left", on="case_id")
)
...
# Additional training data, depth = 2
train_applprev_2 = pl.read_csv(pathway + "csv_files/train/train_applprev_2.csv").pipe(
    set_table_dtypes
)

train_person_2 = pl.read_csv(pathway + "csv_files/train/train_person_2.csv").pipe(
    set_table_dtypes
)
...
train_credit_bureau_a_2 = pl.concat(
    [
        pl.read_csv(
            pathway + "csv_files/train/train_credit_bureau_a_2_0.csv", columns=sel
        ).pipe(set_table_dtypes),
        pl.read_csv(
            pathway + "csv_files/train/train_credit_bureau_a_2_1.csv", columns=sel
        ).pipe(set_table_dtypes),
        pl.read_csv(
            pathway + "csv_files/train/train_credit_bureau_a_2_2.csv", columns=sel
        ).pipe(set_table_dtypes),
        pl.read_csv(
            pathway + "csv_files/train/train_credit_bureau_a_2_3.csv", columns=sel
        ).pipe(set_table_dtypes),
        pl.read_csv(
            pathway + "csv_files/train/train_credit_bureau_a_2_4.csv", columns=sel
        ).pipe(set_table_dtypes),
        pl.read_csv(
            pathway + "csv_files/train/train_credit_bureau_a_2_5.csv", columns=sel
        ).pipe(set_table_dtypes),
    ],
    how="vertical_relaxed",
)

train_credit_bureau_a_21 = pl.concat(
    [
        pl.read_csv(
            pathway + "csv_files/train/train_credit_bureau_a_2_6.csv", columns=sel
        ).pipe(set_table_dtypes),
        pl.read_csv(
            pathway + "csv_files/train/train_credit_bureau_a_2_7.csv", columns=sel
        ).pipe(set_table_dtypes),
        pl.read_csv(
            pathway + "csv_files/train/train_credit_bureau_a_2_8.csv", columns=sel
        ).pipe(set_table_dtypes),
        pl.read_csv(
            pathway + "csv_files/train/train_credit_bureau_a_2_9.csv", columns=sel
        ).pipe(set_table_dtypes),
        pl.read_csv(
            pathway + "csv_files/train/train_credit_bureau_a_2_10.csv", columns=sel
        ).pipe(set_table_dtypes),
    ],
    how="vertical_relaxed",
)

train_applprev_2_feats = (
    train_applprev_2.select(["case_id", "num_group1", "num_group2", "conts_type_509L"])
...
train_credit_bureau_a_21_feats = train_credit_bureau_a_21.group_by("case_id").agg(
    pl.col("pmts_dpd_1073P").sum().alias("pmts_dpd_1073P_sum"),
    pl.col("pmts_dpd_303P").sum().alias("pmts_dpd_303P_sum"),
    pl.col("pmts_overdue_1140A").sum().alias("pmts_overdue_1140A_sum"),
    pl.col("pmts_overdue_1152A").sum().alias("pmts_overdue_1152A_sum"),
...
join_data3 = (
    ...
    .join(train_credit_bureau_a_2_feats, how="left", on="case_id")
    .join(train_credit_bureau_a_21_feats, how="left", on="case_id")
    .join(train_person_2_feats, how="left", on="case_id")
)
...
join_train = reduce(lambda left, right: pd.merge(left, right, on="case_id"), dfs)

# Convert back to polars for datetime
join_train = pl.from_pandas(join_train)

train = join_train.with_columns(pl.col("date_decision").cast(pl.Date))
...
for col in date_list:
    train = train.with_columns(pl.col(col).cast(pl.Date))
...
train = train.pipe(set_table_dtypes).pipe(convert_strings)
...
# del join_data1, join_data2, join_data3, join_train, dfs
# gc.collect()
...
train.drop(columns=drop_list, axis=1, errors="ignore", inplace=True)
